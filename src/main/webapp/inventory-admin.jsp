        ...
        <c:if test="${importResult}">
            <div class="import-result-panel ${importErrorRows > 0 ? 'has-error' : 'all-success'}">
                <div class="import-result-header">
                    ...
                    <c:if test="${not empty googleImportResultSheetUrl}">
                        <a href="${googleImportResultSheetUrl}" target="_blank" class="btn-open-result-sheet">
                ...
                <div class="import-result-note">
                    <c:choose>
                        <c:when test="${importErrorRows > 0}">
                            Có dòng cần kiểm tra nên hệ thống không nhập các dòng đó. Mở tab <strong>KetQuaNhap</strong> để xem lý do và sửa lại dữ liệu trong tab <strong>NhapKho</strong>.
                        </c:when>
                        <c:otherwise>
